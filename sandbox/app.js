  ...
  function viewSheet(key) {
    ...
    return `<div class="veil" data-close="1"><div class="sheet" role="dialog" aria-modal="true">
      ...
      <div class="field note" data-note-for="${esc(key)}">
        <span class="lbl">${w.kind === "album" && !inAlbum ? "История релиза" : "Своими словами"}</span>
        <p class="hint">${w.kind === "album" && !inAlbum
          ? `Общая история альбома: встанет в начало описания у каждого из ${w.tracks.length} треков,
             перед его собственным текстом. Про конкретный трек — в самом треке, в списке выше.`
          : `Появится на странице трека под заголовком и в карточке на сетке —
